...
var pong_game = load("res://scripts/pong/game.tscn")
var game_name = "pong"
...
func start():
	$Start/Label.text = tr("RECORD") + ": " + str(Global.highest_scores.pong)
	
	if (Global.highest_scores.pong < 20):
		$Start/Button2.hide()
	else:
		$Start/Button2.text = tr("START_AT") + ": " + str(int(Global.highest_scores.pong/2))
		$Start/Button2.show()
...
func _on_again_button_pressed():
	game.queue_free()
	Global.main_scene.start_game(pong_game.instantiate())
