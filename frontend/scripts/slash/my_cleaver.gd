...
func init(_room_no, cleaver_data):
	start_pos = Vector3(-0.03015, 0.98481, 0.17101) * cleaver_data.s.z*0.3/2 + Vector3(-0.2, 0.9, 0.2)
	end_pos = Vector3(0.03015, 0.17101, 0.98481) * cleaver_data.s.z*0.3/2 + Vector3(0.7, 0, 0.4)
	room_no = _room_no
...
func on_hud_changed(name, params): 
	if (name == "slash"):
		Hud.get_node("ui/MarginContainer/SlashBtn").connect("pressed", slash)

func slash(): 
	if (not isSlashing): 
		Slash_Server.slash(room_no)
		isSlashing = true 
		t = 0.0
